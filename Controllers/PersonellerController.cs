...
namespace TaramaMVC.Controllers
{
    ...
    public class PersonellerController : Controller
    {
        ...
        [ValidateAntiForgeryToken]
        ////[Authorize]
        public async Task<IActionResult> Edit(int id, [Bind("Id,Name,SurName,AnaBilimDallariId,ScholarName,User,Alintilanma,ApiKey")] Personel personel)
        {
            if (id != personel.Id)
            {
                return NotFound();
        ...
        public async Task<IActionResult> Test()
        {
            ...
            string apiKey = "50c6df0d658a41d5b1767b4a9952ccc027726a125d1eced55e7c3449394c3365";


            // Alıntı yapılan makalenin ID'sini buraya yerleştirin
            var citesId = "13219758673335792490";

            // SerpAPI'nin BASE_URL'ini belirleyin
            var baseUrl = "https://serpapi.com/search";

            // Arama motorunu burada belirleyin
            var searchEngine = "google_scholar";

            // SerpAPI sorgusunu oluşturun
            var queryString = $"api_key={apiKey}&cites={citesId}&engine={searchEngine}";
            ...
            // Yanıtı okuyun ve HtmlAgilityPack ile işleyin
            var responseContent = await httpResponse.Content.ReadAsStringAsync();
            var doc = new HtmlDocument();
            doc.LoadHtml(responseContent);

            // Alıntı yapan makalelerin listesini alın
            var citingArticles = new List<string>();
            foreach (HtmlNode node in doc.DocumentNode.SelectNodes("//div[@class='gs_ri']"))
            {
                citingArticles.Add(node.InnerText);
            }

            // Alıntı yapan makalelerin listesini yazdırın
            foreach (var article in citingArticles)
            {
                sorgu+=(" - "+article);
            }
            return View(sorgu);
